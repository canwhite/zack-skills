...
def render_root_readme(skills: list[dict]) -> str:
    ...
    return (
        ...
        "```\n"
        "\n"
        "Works without uv — `uv run cli.py` is just sugar for the same script. "
        "For the full comparison of all 6 install paths, see [`docs/install-channels.md`](docs/install-channels.md).\n"
        "\n"
