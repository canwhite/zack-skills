...
工程工作流 skill 工具集,面向 Claude Code。当前版本 `VERSION` = 1.9.3。
...
See [`docs/install-paths.md`](./install-paths.md) for the full breakdown of all three mechanisms, their mechanics, and when to use each.
...
**当前局限**: `cli.py` 是项目本地的（通过 `uv run cli.py` 调用），不随 npm 包发布；用户通过 npm 安装拿不到 CLI，需 clone 仓库后使用。
...
├── skills/
│   ├── engineering/<skill>/SKILL.md    # 7 个工程 skill
